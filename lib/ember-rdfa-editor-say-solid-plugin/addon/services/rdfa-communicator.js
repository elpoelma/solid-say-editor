...
export default class RdfaCommunicatorService extends Service {
    ...
    async generateRDFaTag({ predicate, object }) {
        ...
        if (object.termType === "Literal") {
            ...
            return `<a property="${predicate.value}" href="${object.value}" title="${metadata[RDFS("comment").value]}"> ${object.value} </span>`;
        }
    }
    ...
    fromRDFa(block, rdfa) {
        rdfa.forEach(quad => {
            if (quad.predicate !== "a" && quad.subject !== "#") {
     ...
     */
    fetchTriples(subject, attributes, graph, options) {
        return new Promise((resolve, reject) => {
            ...
            this.store.fetcher.nowOrWhenFetched(graph, async (ok, body, xhr) => {
                if (ok) {
                    let result_quads = []
                    ...
                    await this.store.fetcher.load("https://proxy.linkeddatafragments.org/" + xhr.url).then(res => {
                        console.log(res);
                        let result_quads = []
                        ...
                        console.log(result_quads);
    
    
                        resolve(result_quads);
                    }).catch(err => {throw(new Error("Could not load quads"))});
    ...
    fetchRemoteQuads(subject, attributes, graph){
        return new Promise((resolve, reject) => {
            this.fetchTriples(subject, attributes, graph, { ProxyUri: "https://proxy.linkeddatafragments.org/", proxyUsed: true}).then(quads => {
                resolve(quads);
            }).catch(err => {
                resolve([]);
            })
